...
//               System.out.println("pid: "+pid+"pname: "+pname+"price: "+price+"stock: "+stock);
    ...
    public void updateTicket(Ticket ticket) {
        ...
        try {
            ...
            try {
                ...
                String sql = "update tickets set tname=?,tprice=?,tkind=?,timg=? where ticID =? ";
                //获取数据库预操作对象
                ps = conn.prepareStatement(sql);
    ...
    public void addTicket(Ticket ticket) {
        ...
        try {
            ...
            try {
                conn.setAutoCommit(false);
                String sql = "INSERT INTO tickets(tname,tprice,tkind,timg) VALUE(?,?,?,?)";
                ps = conn.prepareStatement(sql);
                ps.setString(1, ticket.getTname());
                ps.setDouble(2, ticket.getTprice());
                ps.setString(3, ticket.getTkind());
...
// 出现错误，保证关闭的try
        ...
        try {
            ...
            String sql = "SELECT ticID,tname,tprice,tkind,timg FROM tickets WHERE ticID=?";
// 获取数据库预操作对象
            ps = conn.prepareStatement(sql);
            ps.setInt(1, ticID);
            rs = ps.executeQuery();
            if (rs.next()){
                ...
                ticket = new Ticket(ticID,tname,tprice,tkind,timg);
            }
            else return null;
        } catch (SQLException e) {
            e.printStackTrace();
...
// 出现错误，保证关闭的try
        try {
            conn = DBUtil.getConnection();
            String sql = "SELECT ticID,tname,tprice,tkind,timg FROM tickets WHERE tname like ?";
// 获取数据库预操作对象
            ps = conn.prepareStatement(sql);
            ...
            while (rs.next()){
                ...
                String tkind=rs.getString("tkind");
                String timg=rs.getString("timg");
                list.add(new Ticket(ticID,tname,tprice,tkind,timg));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
